...
package org.danmayr.imagej.shape_detector.cellMagicWand;
...
public class PolarTransform {
	...
	public ArrayList<CartesianPixel> connectPolarPixels(PolarPixel a, PolarPixel b){
		...
		CartesianPixel aEnd = new CartesianPixel(a.r, a.thetaEnd-Constants.EPSILON, cx, cy, originalImage.getWidth()-1, originalImage.getHeight()-1);
	...
	public int[][] getEdgePointsForRoi(){
		...
		for(int i=0; i < edgePointsX.size(); i++){
			edgePoints[0][i] = edgePointsX.get(i);
			edgePoints[1][i] = edgePointsY.get(i);
		}
		return edgePoints;
	}
	...
	public ArrayList<Pixel> getEdgePoints(){
		...
		for(int i=0; i<pixelCellEdge.size(); i++){
			edge.add(new Pixel(pixelCellEdge.get(i).x,pixelCellEdge.get(i).y));
		}
	...
	public ArrayList<Pixel> getEdgePoints(){
		...
		ArrayList<Pixel> edgePointsCulled = new ArrayList<Pixel>();
		edgePointsCulled.add(edgePoints.get(0));
		int j = 0;
		for(int i = 1; i < edgePoints.size(); i++){
			//skip to the next new point
			while(i < edgePoints.size() && edgePoints.get(i).x == edgePointsCulled.get(j).x && edgePoints.get(i).y == edgePointsCulled.get(j).y){
				i++;
			}
			...
			//add it
			edgePointsCulled.add(edgePoints.get(i));
			j++;
		}
		
		return edgePointsCulled;
	}
	*/
